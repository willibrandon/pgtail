// Package instance provides the PostgreSQL instance type and detection sources.
package instance
...
const (
	// SourceProcess indicates the instance was found via running postgres process.
...
type Instance struct {
	...
	// Running indicates whether the postmaster is currently running.
	Running bool
	...
	// SourceDetail provides additional context about the source (e.g., "brew" for SourceKnownPath).
	SourceDetail string
}

// DisplaySource returns the display string for the instance's source.
// It uses SourceDetail if available for known paths, otherwise the Source string.
